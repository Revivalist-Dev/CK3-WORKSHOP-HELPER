...
class ModProcessor:
    ...
    async def initialize(self) -> None:
        ...
        await logger.info(f"Using output directory: {self.output_path}")
        await logger.info(f"Using local mods directory: {self.config['local_mods_path']}")

    async def process_single_mod_file(self, mod_file_path: str, source_path: str, is_local: bool) -> bool:
        try:
            mod_folder_path = Path(source_path) / mod_file_path
            metadata = await ModValidator.validate_mod(mod_file_path, str(mod_folder_path))

            if not metadata:
                await logger.warn(f"Failed to validate mod: {mod_file_path}")
                return False
            ...
            is_game_version_compatible = await ModValidator.check_game_version(metadata, self.GAME_VERSION)
            if not is_game_version_compatible:
                await logger.warn(f"Mod {metadata['name']} may not be compatible with game version {self.GAME_VERSION}")

            # Check dependencies
            if metadata.get("dependencies"):
                for dep in metadata["dependencies"]:
                    if not await ModValidator.validate_dependencies(metadata, dep):
            ...
            await logger.info(f"Processing {'local' if is_local else 'workshop'} mod: {metadata['name']} ({version_string})")

            # Check if mod already exists
            if dest_path.exists():
                await logger.info(f"Mod already exists: {safe_name}")
                return True
            ...
            try:
                ...
                for file in existing_files:
                    await FileOperations.create_backup(self.output_path / file)
            except FileNotFoundError:
                pass

            # Copy mod files with verification
            success = await FileOperations.copy_dir_concurrent(
                str(mod_folder_path),
                str(dest_path),
            ...
            if success:
                await logger.info(f"Successfully processed mod: {safe_name}")
                return True
            else:
                await logger.error(f"Failed to process mod: {safe_name}")
                return False
        except Exception as error:
            await logger.error(f"Error processing mod: {mod_file_path}", error)
            return False

    async def process_mods_in_directory(self, directory: str, is_local: bool) -> Tuple[int, int]:
        ...
        for file in mod_files:
            if await self.process_single_mod_file(file, directory, is_local):
                successful += 1
            processed += 1
    ...
    async def process_all_mods(self) -> None:
        try:
            await logger.info('Starting mod processing...')

            # Process Workshop mods
            workshop_processed, workshop_successful = await self.process_mods_in_directory(
                self.config["workshop_path"],
                False
            )
            await logger.info(f"Processed {workshop_successful}/{workshop_processed} workshop mods")

            # Process local mods
            local_processed, local_successful = await self.process_mods_in_directory(
                self.config["local_mods_path"],
                True
            )
            await logger.info(f"Processed {local_successful}/{local_processed} local mods")

            # Generate the mod issues report
            await self.mod_report.generate_report()

            # Cleanup old backups and logs
            await FileOperations.cleanup(str(self.output_path), 30)  # Keep backups for 30 days
            await logger.cleanup(7)  # Keep logs for 7 days

            await logger.info('Completed processing all mods')
        except Exception as error:
            await logger.error('Error processing mods', error)
            sys.exit(1)
...
async def main():
    try:
        ...
        await processor.process_all_mods()
    except Exception as error:
